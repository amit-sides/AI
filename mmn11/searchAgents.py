  ...
  def getSuccessors(self, state):
    ...
    for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
      ...
      if not self.walls[nextx][nexty]:
        ...
        # If we reached a corner, remove it from corners list
...
def smallest_distance_of_3_points(current_position, points, problem=None):
  """
  Receives the current position and a list of 3 (or less) points.
  Calculates the minimum manhattan distance from current position to all points.
  """
  ...
  # Calculates the optimal (theoretical) distance to reach all 3 points
  min_distance_of_current_combination = float("Inf")
  for i in range(len(points)):
    # d1 + d2 + d3 will be the shortest manhattan distance to all 3 points from current position, where point `i` is the first we go to
    ...
    # If going to point `i` first is better than all previous points, we keep this as the best minimum distance
...
def smallest_distance_of_4_points(current_position, points, problem=None):
  """
  Receives the current position and a list of 4 (or less) points.
  Calculates the minimum manhattan distance from current position to all points.
  """
  if len(points) <= 3:
    return smallest_distance_of_3_points(current_position, points, problem)

  min_distance = float("Inf")
  for i in range(len(points)):
    points_left = [points[i - 1], points[i - 2], points[i - 3]]

    # Calculates the minimum distance to reach point `i` from current position, and from there go to all other points.
...
def foodHeuristic(state, problem):
  ...
  # So, when our food list gets smaller than 9, we check combinations of 4 instead of 3.
  amount = 3 if len(food_list) > 9 else 4
  """
  amount = 3  # Using fixed amount (3) gives an optimal solution in about 6 seconds with ~7040 nodes.
              # Using dynamic amount (like in the previous commented line) gives an optimal solution in about 12 seconds with ~6980 nodes.
